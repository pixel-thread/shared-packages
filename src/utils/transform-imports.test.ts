...
describe('transformImports', () => {
  it('transforms static imports', () => {
    ...
    const expected = `import { ApiError } from '@/src/shared/errors/api-error';`;
  ...
  it('transforms multiple imports in the same file', () => {
    ...
    const expected = [
      ...
      `import { PAGE_SIZE } from '@/src/shared/shared-constants';`,
    ].join('\n');
    expect(transformImports(input, sampleConfig)).toBe(expected);
  });
  ...
  it('handles a mix of transformed and untouched imports', () => {
    ...
    expect(result).toContain(`import { ApiError } from '@/src/shared/errors/api-error'`);
  ...
  it('preserves leading src/ in the consumer path', () => {
    const input = `import { Provider } from '@services/storage-provider';`;
    const expected = `import { Provider } from '@/src/services/storage-provider';`;
    expect(transformImports(input, sampleConfig)).toBe(expected);
  });
  ...
  it('handles hyphenated path keys like services-storage', () => {
    ...
    const expected = `import { StorageProvider } from '@/src/shared/services/storage/types';`;
  ...
  it('preserves trailing src/ in the consumer path', () => {
    const config: ConsumerConfig = {
      alias: '@',
      aliases: {},
      paths: {
    ...
    const expected = `import { format } from '@/src/shared/utils/format-zod-issues';`;
    expect(transformImports(input, config)).toBe(expected);
  });
  ...
  describe('internal prefix handling', () => {
    ...
    it('handles @items/utils/ prefix', () => {
      ...
      const expected = `import { formatZodIssues } from '@/src/shared/utils/format-zod-issues';`;
      expect(transformImports(input, config)).toBe(expected);
    });
